...
int main(int argc, char* argv[]) {
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " <width> <height> <value_for_addition> <value_for_subtraction> <value_for_multiplication>" << std::endl;
        return 1;
    }
    ...
    // Create random array on host
    std::srand(std::time(nullptr));
    int* h_input = new int[size];
    for (int i = 0; i < size; i++) {
        h_input[i] = rand() % 100;  // Random numbers between 0-99
    }
    ...
    printMatrix(h_output, width, height, "Final Matrix (after addition, subtraction, multiplication)");
